...
void enumerate_function(u8 bus, u8 device, u8 function) {
    ...
    u16 class = pci_read_register8(bus, device, function, PCI_CLASS_CODE_REG_OFFSET);
    ...
    n = sprintf(class_str, "%x ", class);
    strcpy(class_str + n, pci_class_strings[class]);
    n = sprintf(subclass_str, "%x ", subclass);
    strcpy(subclass_str + n, (*pci_subclass_strings[class])[subclass]);
    ...
    if (class == BridgeDevice && subclass == PCI_TO_PCI) {
